...
$(function() {
  ...
  function redraw() {
    ...
    if (controls) {
      ...
      cutter.drawTile(dst_ctx, 0, 0, dst_cvs.width, dst_cvs.height, triangle, function(ctx) {
  ...
  function showViewer() {
    ...
    var width = 2560;
    var height = 1440;
    var img = cutter.makeImage(triangle, function(ctx) {
      zoom.setupContext(ctx);
      ctx.drawImage(image, 0, 0);
    },
    width, height);
  ...
  $destination.click(function(e) {
    ...
    if (triangle) {
      ...
      setImage(cutter.makeImage(triangle, function(ctx) {
        zoom.setupContext(ctx);
        ctx.drawImage(image, 0, 0);
      },
      size, size));
