...
$(function() {
  ...
  function redraw() {
    ...
    if (controls) {
      ...
      var cutting = cutter.cutTriangle(triangle, function(ctx) {
        zoom.setupContext(ctx);
        ctx.drawImage(image, 0, 0);
      });

      cutter.tile(cutting, dst_ctx, dst_cvs.width, dst_cvs.height, dst_cvs.width / 2, dst_cvs.height / 2);
  ...
  function makeControls() {
    ...
    zoom = new ZoomPan(image.width, image.height);
  ...
  function showViewer() {
    ...
    var width = 2560;
    var height = 1440;
    var img = cutter.makeImage(cutting, width, height);
  ...
  $destination.click(function(e) {
    ...
    if (triangle) {
      ...
      setImage(cutter.makeImage(cutting, size, size));
    }
  });
